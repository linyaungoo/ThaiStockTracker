...
function App() {
  ...
  const [liveResult] = useState<LiveResult>({
    set: "1,100.01",
    value: "52,840.03",
    time: "16:31:58",
    twod: "10"
  ...
  const [todayResults] = useState<TodayResult[]>([
    {
      set: "1,091.08",
      value: "24,231.22",
      open_time: "11:00:00",
    ...
    <div style={{ maxWidth: '480px', margin: '0 auto', backgroundColor: '#ffffff', minHeight: '100vh', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)', color: 'white', margin: '16px', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button onClick={handleRefresh} disabled={isLoading} style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
            {isLoading ? '⟳' : '↻'} Refresh
          </button>
        </div>
      ...
      <div style={{ textAlign: 'center', padding: '16px' }}>
        <p style={{ fontSize: '12px', color: '#6b7280' }}>
          {isLoading ? 'Refreshing display...' : 'UI Demo - No live data connection'}
        </p>
      </div>
    </div>
  );
